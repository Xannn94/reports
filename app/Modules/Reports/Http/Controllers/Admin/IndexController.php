...
class IndexController extends Admin
{
    ...
    public function index()
    {
        ...
        $user   = DB::connection('redmine')->table('users')->where('login',$name)->first();
        $from   = null;
        $to     = null;
    ...
    private function getOptimisms($ticketsId){
        /* Вытаскиваем все оценки оптимизма для нашего пользователя */
        $result = DB::connection('redmine')
            ->table('time_entries')
            ->select('custom_values.value','custom_values.customized_id')
            ->join('custom_values', 'custom_values.customized_id', '=', 'time_entries.issue_id')
            ->whereIn('custom_values.customized_id', $ticketsId)
            ->where('custom_values.custom_field_id',6)->get();
        /* перебираем полученый результат в удобный массив */
        $result->map(function ($item) {
            if($item->value){
                $this->optimism[$item->customized_id] = (float)$item->value;
            }
            else{
                $this->optimism[$item->customized_id] = 0;
            }
        });
    }

    private function getPessimisms($ticketsId)
    {
        /* Вытаскиваем все оценки пессимизма для нашего пользователя */
        $result = DB::connection('redmine')
            ->table('time_entries')
            ->select('custom_values.value','custom_values.customized_id')
            ->join('custom_values', 'custom_values.customized_id', '=', 'time_entries.issue_id')
            ->whereIn('custom_values.customized_id', $ticketsId)
            ->where('custom_values.custom_field_id',7)->get();
        /* перебираем полученый результат в удобный массив */
        $result->map(function ($item) {
            if($item->value){
                $this->pessimism[$item->customized_id] = (float)$item->value;
            }
            else{
                $this->pessimism[$item->customized_id] = 0;
            }
        });
    }

    private function getTotals($ticketsId)
    {
        $result = DB::connection('redmine')
            ->table('time_entries')
            ->select('hours','custom_values.customized_id')
            ->join('custom_values', 'custom_values.customized_id', '=', 'time_entries.issue_id')
            ->whereIn('custom_values.customized_id', $ticketsId)
            ->where('custom_values.custom_field_id',7)->get();
        ...
        $result->map(function ($item) {
            if($item->hours){
                $this->total[$item->customized_id] = (float)$item->hours;
            }
            else{
                $this->total[$item->customized_id] = 0;
            }
        });
    }
